...
class User extends Authenticatable
{
    ...
    public function thumbnail()
    {
        ...
        $avatar = $this->avatar();
        $image = (!$avatar || $avatar->image == 'default')
            ? config('site.renderer.default_filename')
            : $avatar->image;
    ...
    public function render()
    {
        ...
        $request = $renderer->emptyAvatar($this->id);
        $request['items'] = [
            'face' => (int) ($avatar->face ?? 0),
            'hats' => [
                (int) ($avatar->hat_1 ?? 0),
                (int) ($avatar->hat_2 ?? 0),
                (int) ($avatar->hat_3 ?? 0),
            ...
            'head' => (int) ($avatar->head ?? 0),
            'tool' => (int) ($avatar->tool ?? 0),
            'pants' => (int) ($avatar->pants ?? 0),
            'shirt' => (int) ($avatar->shirt ?? 0),
            'figure' => (int) ($avatar->figure ?? 0),
            'tshirt' => (int) ($avatar->tshirt ?? 0)
        ];
        $request['colors'] = [
            'head' => ltrim($avatar->color_head, '#'),
            'torso' => ltrim($avatar->color_torso, '#'),
            'left_arm' => ltrim($avatar->color_left_arm, '#'),
            'right_arm' => ltrim($avatar->color_right_arm, '#'),
            'left_leg' => ltrim($avatar->color_left_leg, '#'),
            'right_leg' => ltrim($avatar->color_right_leg, '#')
        ];
        $image = $renderer->render($request);

        if ($image) {
            $storage = Storage::disk('local');
			$storage->put("thumbnails/avatars/{$thumbnail}.png", base64_decode($image));
        ...
        $this->rendererError = $renderer->lastError();
