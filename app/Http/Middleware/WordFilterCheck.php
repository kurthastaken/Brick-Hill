...
class WordFilterCheck
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($enabled && $request->isMethod('post') && (array_key_exists($url, $this::API_FILTER_URLS) || array_key_exists($route, $this::FILTER_ROUTES))) {
            $api = $request->is('api/*');
            $params = (!$api) ? $this::FILTER_ROUTES[$route] : $this::API_FILTER_URLS[$url];

            foreach ($params as $route => $param) {
                if ($this->isProfanity($request->$param)) {
                    if (!$api)
                        return back()->withErrors(['One or more words have triggered our profanity filter. Please update and try again.']);
                    else
                        return response()->json(['error' => 'One or more words have triggered our profanity filter. Please update and try again.']);
                }
            }
        }
    ...
    public function isProfanity($content)
    {
        $blacklistWords = config('filter_blacklist');
        $whitelistWords = config('filter_whitelist');
        $whitelistDomains = config('filter_whitelist_domains');
        $whitelistDomains[] = config('site.route_domains.main_site');
        $whitelistDomains[] = config('site.route_domains.jobs_site');
        ...
        foreach ($blacklistWords as $blword) {
            if ((stripos($tempContent, $blword) !== false || stripos($content, $blword) !== false) && ($this->arrayInString($whitelistWords, $content) == 0)) {
                $flag = true;
        ...
        $domainContent = str_replace('https://', '', $domainContent);
        $domainContent = str_replace('http://', '', $domainContent);
        ...
        if ($flag) {
            // Log later...
            // implode('|', $violatingWords)
        }

        return $flag;
    }

    protected function arrayInString($inArray, $inString)
    {
        if (is_array($inArray)) {
